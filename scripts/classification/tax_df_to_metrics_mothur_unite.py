    ...
    for r in ["gen", "fam"]:
        for c in ["wang", "knn"]:
            for region in region_dict.keys():
                print(k, r, c, region)
                comb_tax = pd.read_table(F"{args.dir}/sorted_unite_mothur_{c}_{k}_query_{r}{region_dict[region][1]}_comb.txt")
                cons = comb_tax.iloc[:,1:]
                cons = cons.fillna("-")

                name_dict = {}
                with open(F"{args.dir}/sorted_unite_mothur_{c}_{k}_query_{r}{region_dict[region][1]}_comb.txt", "r") as ifile:
                    line = ifile.readline()
                    line = ifile.readline()
                    while line != "":
                        name = line.split("\t")[0]
                ...
                buffer = F"{buffer}unite,{r},{region},{k},mothur-{c},{','.join(metrics)}\n"
with open(F"{args.dir}/part_cv_metrics_unite_mothur.csv", "w") as ofile:
    ofile.write(buffer)
