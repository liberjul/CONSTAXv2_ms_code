    ...
    for j in range(5):
        for d in ["unite"]: #, "silva"]:
            for region in region_dict.keys():
                ...
                comb_tax = pd.read_table(F"{args.dir}/out_p{j}_{i}_u{region_dict[region][0]}/combined_taxonomy.txt")
                rdp = comb_tax.iloc[:,1::4]
                sintax = comb_tax.iloc[:,2::4]
                utax = comb_tax.iloc[:,3::4]
                cons = comb_tax.iloc[:,4::4]
                rdp, sintax, utax, cons = [x.fillna("-") for x in [rdp, sintax, utax, cons]]

                name_dict = {}
                with open(F"{args.dir}/out_p{j}_{i}_u{region_dict[region][0]}/combined_taxonomy.txt", "r") as ifile:
                    line = ifile.readline()
                    line = ifile.readline()
                    while line != "":
                        name = line.split("|")[1]
                ...
                for pred_df in [rdp, sintax, utax, cons]:
...
with open(F"{args.dir}/part_cv_metrics_unite_reg_utax.csv", "w") as ofile:
    ofile.write(buffer)
buffer = "database,partition_level,region,k_iteration,classifier,sensitivity,MC,OC,EPQ,N_known,N_novel,N\n"
for i in ["fam", "gen"]:
    for j in range(5):
        for d in ["unite"]: #, "silva"]:
            for region in region_dict.keys():
                comb_tax = pd.read_table(F"{args.dir}/out_p{j}_{i}_u{region_dict[region][0]}_cons/combined_taxonomy.txt")
                rdp = comb_tax.iloc[:,1::4]
                sintax = comb_tax.iloc[:,2::4]
                utax = comb_tax.iloc[:,3::4]
                cons = comb_tax.iloc[:,4::4]
                ...
                name_dict = {}
                with open(F"{args.dir}/out_p{j}_{i}_u{region_dict[region][0]}_cons/combined_taxonomy.txt", "r") as ifile:
                    line = ifile.readline()
                    line = ifile.readline()
                    while line != "":
                        name = line.split("|")[1]
                ...
                for pred_df in [rdp, sintax, utax, cons]:
                    ...
                    classifier = pred_df.columns[0].split("_")[1]
                    buffer = F"{buffer}{d},{i},{region},{j},{classifier},{','.join(metrics)}\n"
with open(F"{args.dir}/part_cv_metrics_unite_reg_utax_conservative.csv", "w") as ofile:
    ofile.write(buffer)
