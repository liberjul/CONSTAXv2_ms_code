...
region_dict = {"Full" : "_full", "V4" : "_v4", "V3-4" : "_v3-4"}
for k in range(5):
    for r in ["gen", "fam"]:
        for c in ["wang", "knn"]:
            for region in region_dict.keys():
                print(k, r, c, region)
                comb_tax = pd.read_table(F"{args.dir}/sorted_silva_mothur_{c}_{k}_query_{r}{region_dict[region]}_comb.txt")
                cons = comb_tax.iloc[:,1:]
                cons = cons.fillna("-")

                name_dict = {}
                with open(F"{args.dir}/sorted_silva_mothur_{c}_{k}_query_{r}{region_dict[region]}_comb.txt", "r") as ifile:
                    line = ifile.readline()
                    line = ifile.readline()
                    while line != "":
                        name = line.split("\t")[0]
                        ...
                        line = ifile.readline()
                if region == "Full":
                    tax_file = F"{args.dir}/query_dbs/silva_partition_{k}_query_sub_1k_{r}__RDP_taxonomy.txt"
                else:
                    tax_file = F"{args.dir}/query_dbs/silva_partition_{k}_query_{r}{region_dict[region]}__RDP_taxonomy.txt"
                ...
                buffer = F"{buffer}silva,{r},{region},{k},mothur-{c},{','.join(metrics)}\n"
with open(F"{args.dir}/part_cv_metrics_silva_mothur.csv", "w") as ofile:
    ofile.write(buffer)
